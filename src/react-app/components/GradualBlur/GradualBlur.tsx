...
const useResponsiveDimension = (responsive: boolean, config: GradualBlurProps, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${keyCap}` as keyof GradualBlurProps])
        v = config[`mobile${keyCap}` as keyof GradualBlurProps] as string;
      else if (w <= 768 && config[`tablet${keyCap}` as keyof GradualBlurProps])
        v = config[`tablet${keyCap}` as keyof GradualBlurProps] as string;
      else if (w <= 1024 && config[`desktop${keyCap}` as keyof GradualBlurProps])
        v = config[`desktop${keyCap}` as keyof GradualBlurProps] as string;
      setValue(v);
    };
