...
function Lens({ modeProps, children }: WrapperProps) {
  return <ModeWrapper glb="/assets/3d/lens.glb" geometryKey="Cylinder" followPointer modeProps={modeProps}>{children}</ModeWrapper>;
}

function Cube({ modeProps, children }: WrapperProps) {
  return <ModeWrapper glb="/assets/3d/cube.glb" geometryKey="Cube" followPointer modeProps={modeProps}>{children}</ModeWrapper>;
}

function Bar({ modeProps = {}, children }: WrapperProps) {
  const defaultMat = {
    transmission: 1,
    roughness: 0,
    thickness: 10,
...
function NavItems({ items }: { items: Array<{ label: string, link: string }> }) {
  ...
  useFrame(() => {
    ...
    group.current.children.forEach((child, i) => {
      (child as THREE.Object3D).position.x = (i - (items.length - 1) / 2) * spacing;
    });
  });
